...
def run_batch_download(
    ...
    """批量下载视频。返回统计信息，包含 failed_ids 列表。"""
    ...
    if not video_data:
        print("未找到任何视频")
        return {"success": 0, "fail": 0, "skip": 0, "total_size": 0, "failed_ids": []}

    total_to_download = len(video_data)
    print(f"共 {total_to_download} 个视频待下载\n")

    downloaded_log = output_path / "downloaded.txt"
    downloaded_ids = set()
    ...
    print(f"待下载: {total_pending} 个\n")

    if not pending:
        print("全部已下载，无需操作")
        print(f"  保存至: {output_path.resolve()}/")
        return {"success": 0, "fail": 0, "skip": skip, "total_size": 0, "failed_ids": []}
    ...
    def download_one(item: dict) -> None:
        ...
        try:
            ...
            with lock:
                ...
                try:
                    print(f"[{done_count}/{total_pending}] {filename} ({size / (1024*1024):.1f} MB)")
                except UnicodeEncodeError:
                    print(f"[{done_count}/{total_pending}] {video_id} ({size / (1024*1024):.1f} MB)")
            ...
            with lock:
                ...
                failed_ids.append(video_id)
                done_count += 1
                print(f"[{done_count}/{total_pending}] {video_id}  错误: {e}")

    workers = max(1, min(threads, total_pending))
    ...
    # 写失败 ID 列表
    if failed_ids:
        failed_file = output_path / "failed.txt"
...
def main():
    ...
    run_batch_download(video_data, output_dir, threads, date_from, date_to)
